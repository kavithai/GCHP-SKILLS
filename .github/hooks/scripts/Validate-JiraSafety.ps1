try {
    ...
    switch ($toolName) {
        ...
        'create_file' {
            ...
            # Category 4 — Bypass script creation
            $content = $toolInput.content
            if ($content) {
                if (($content -imatch 'DELETE') -and ($content -imatch 'jira')) {
                    Write-HookResult -Decision 'deny' -Reason 'Blocked creation of file containing DELETE + Jira references (potential bypass script).'
                    exit 0
                }
                if ($content -imatch 'Invoke-RestMethod.*Delete') {
